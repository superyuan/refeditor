...
{
	GenomePosition gp;
	gp.pos = abs(atoi(position.c_str()));
	gp.chrName = name;
...
//int CLineFields::divide2(string s, string divisor, vector<string> &field)
int CLineFields::divide2(string s, string divisor)
{
	int l=s.length();
	int j=0;
	for (int i=0; i<l; i++) {
		if (s[i] != '\t') {
			if (sect_num<FIELD_NUM && j<FIELD_SIZE) {
				sect[sect_num][j]=s[i];
				j++;
			}
		} else {
...
CLineFields::CLineFields()
{
	...
	fp = NULL;
	sect_num = 0;
	sect = new char *[FIELD_NUM];
	for (int i=0; i<FIELD_NUM; i++)
		sect[i] = new char[FIELD_SIZE];
...
bool CLineFields::readline (bool ignoreComment, string divisor)
{
	...
	divide2(line, divisor);
	//divide2(line, divisor, field);
	//	istringstream iss(line);
	//	copy(istream_iterator<string>(iss),
	//		istream_iterator<string>(),
	//		back_inserter<vector<string> >(field));
